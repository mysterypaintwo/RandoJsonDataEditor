...
ipcMain.on('open-room-properties-editor', (event, roomPropertiesData, enemyList, itemList, eventList, weaponList, techMap, helperMap) => {
    ...
    roomPropertiesWin.on('close', (e) => {
        // We need to check with the renderer if there are unsaved changes
        // The renderer will handle showing the confirmation dialog via beforeunload
        // So we don't need to do anything here - the beforeunload will prevent close if needed
    });
